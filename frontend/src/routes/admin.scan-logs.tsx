...
function ScanLogsPage() {
  ...
  const dateParam =
    dateMode === "today" ? todayISO() : dateMode === "custom" ? customDate : undefined;
        ...
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
            ...
            <Card className="p-4 shadow-sm border col-span-2 sm:col-span-1">
              ...
              <div className="font-display text-3xl font-bold mt-1">{todayLogs.length}</div>
            </Card>
          </div>
        ...
        <Card className="shadow-sm border animate-fade-in overflow-hidden">
          ...
          <div className="max-h-[600px] overflow-y-auto hidden md:block">
            <table className="w-full text-sm">
              ...
              <tbody>
                  ...
                  <tr key={l.id} className="border-t hover:bg-muted/5 transition-all">
                    ...
                    <td className="px-4 py-2 text-xs">
                      {l.denialCode && (
                        <Badge
                          variant="outline"
                          className="mr-1 text-[10px] font-bold border-destructive/30 text-destructive"
                        >
                          {l.denialCode}
                        </Badge>
                      )}
                ...
                {!allQ.isLoading && filtered.length === 0 && (
                  <tr>
          ...
          <div className="grid grid-cols-1 gap-3 p-4 md:hidden max-h-[600px] overflow-y-auto">
            ...
            {filtered.map((l) => (
              <div
                key={l.id}
                className="flex flex-col gap-2 rounded-xl border bg-card p-3 shadow-sm hover:bg-muted/5 transition-all"
              >
                ...
                {l.status === "denied" && (
                  <div className="mt-1 rounded-md bg-destructive/5 p-2 text-[10px]">
                    {l.denialCode && (
                      <Badge
                        variant="outline"
                        className="mr-1 text-[9px] border-destructive/30 text-destructive font-bold"
                      >
                        {l.denialCode}
                      </Badge>
                    )}
            ...
            {!allQ.isLoading && filtered.length === 0 && (
              <div className="py-8 text-center text-sm text-muted-foreground">No scan logs yet</div>
            )}
          </div>
        </Card>
      )}
