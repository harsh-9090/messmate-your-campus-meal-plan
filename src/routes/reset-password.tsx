      ...
      <div className="relative hidden flex-col justify-between bg-sidebar p-10 text-sidebar-foreground lg:flex overflow-hidden">
        ...
        >
          ...
          <p className="max-w-md text-sidebar-foreground/70 text-lg leading-relaxed">
            Ensure your account remains safe. Select a strong password that is easy for you to remember but hard for others to guess.
          </p>
        </motion.div>
        ...
        >
          {!token || !memberId ? (
            <div className="space-y-4 text-center">
              <h2 className="font-display text-3xl font-extrabold text-destructive">Invalid Reset Link</h2>
              <p className="text-muted-foreground text-sm font-medium">
                This password reset link is missing parameters. Please request a new link from the forgot password screen.
              </p>
              <Button onClick={() => navigate({ to: "/forgot-password" })} className="w-full h-14 rounded-2xl mt-4">
                Request New Link
              </Button>
            </div>
          ) : !success ? (
            <>
              <div className="space-y-3">
                <h2 className="font-display text-4xl lg:text-5xl font-extrabold tracking-tight">Set Password</h2>
                <p className="text-muted-foreground font-medium">Configure new credentials for ID: <strong className="text-foreground">{memberId}</strong></p>
              </div>

              <form className="space-y-6" onSubmit={handleReset}>
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="pw" className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/80 ml-1">
                      New Password
                    </Label>
                    <Input
                      id="pw"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="cpw" className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/80 ml-1">
                ...
                <Button type="submit" className="w-full h-14 text-lg font-bold rounded-2xl shadow-xl shadow-primary/20 transition-all active:scale-[0.98]" disabled={submitting}>
                  {submitting ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : <Lock className="mr-2 h-5 w-5" />}
                  Change Password
                </Button>
              </form>
            </>
            ...
            >
              ...
              <div className="space-y-3">
                <h2 className="font-display text-3xl font-extrabold tracking-tight">Reset Complete!</h2>
                <p className="text-muted-foreground font-medium text-sm leading-relaxed">
                  Your password has been successfully updated. We are redirecting you to the sign in page so you can access your dashboard...
                </p>
              </div>
